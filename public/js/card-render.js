import { BRANDS, FALLBACK_BRAND_ID } from '/shared/cards.js';

/** Multicolor Google G for corporate demo (?logo) only. */
...
function isCorporateLogoDemo() {
  return Boolean(window.__TAKE5_LOGO_DEMO__);
}
...
export function logoUrl(brandId) {
  if (isCorporateLogoDemo() && brandId === 'google') {
    return `data:image/svg+xml,${encodeURIComponent(GOOGLE_LOGO_SVG)}`;
  }
  const brand = BRANDS[brandId];
  const base = brand ? `/assets/logos/${brand.logo}` : `/assets/logos/${BRANDS[FALLBACK_BRAND_ID]?.logo ?? 'logo-2.svg'}`;
...
export function createLogoImg(brandId, className = '') {
  const brand = BRANDS[brandId] ?? BRANDS[FALLBACK_BRAND_ID];
...
export function cardBrandId(cardId, catalog) {
  ...
  return card?.brandId ?? FALLBACK_BRAND_ID;
}
